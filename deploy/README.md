...
git clone https://github.com/ducks/agent-zen-garden.git ~/agent-zen-garden
cd ~/agent-zen-garden
...
the flake automatically. It assumes user `ducks` and `~/agent-zen-garden`.

```bash
sudo cp ~/agent-zen-garden/deploy/agent-zen-garden.service \
...
```bash
cat ~/agent-zen-garden/deploy/Caddyfile.snippet | sudo tee -a /etc/caddy/Caddyfile
